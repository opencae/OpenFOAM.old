...
Gauss積分を使ってLaplace演算子を評価する際にも必要となります．
...
\subsection{Laplace演算子スキーム}
\label{ssec:4.4.4}
\OFsubdictionary{laplacianSchemes}サブディクショナリにはLaplace演算子項を記述します．
流体力学の中で見られる$\nabla \cdot (\rho\nabla\bm{U})$といった典型的なLaplace演算子項を
どのようにエントリに記述するかというと，
...
スキームの選択は数値的性質を決定します．
先の例でのLaplace演算子項の典型的なエントリは以下のようになります．
\begin{OFverbatim}[file]
