...
\OFdictionary{fvSolution}ディクショナリファイルからの抜粋すると二つのディクショナリが含まれており，
solversとPISOがあります．solversディクショナリは
圧力方程式と速度方程式に対してそれぞれ計算用と収束用に複数のデータ入力があり，
それらはそれぞれ\OFkeyword{p}と\OFkeyword{U}のキーワードによって表現される
...
データは\OFkeyword{dimentions}の入力から始ります．
...
強制的な入力である\OFkeyword{type}は，そのフィールドに合うパッチのフィールド条件を記述します．
...
\end{OFverbatim}
あくまでもこれはどのように機能が働くかを提検証しただけの，単純な例です．
しかしこの機能はユーザのニーズに合うケースデータを一般化する手段として，
...
\OFsubdictionary{solvers}内の各入力の構文はキーワードを使用し，
特定の方程式で解かれれいる変数に関連する\OFclass{word}が用いられる．
例えば\OFtool{icoFoam}は，速度$\bm{U}$と圧力$p$の方程式を解くので，
は\OFkeyword{U}および\OFkeyword{p}となります．
...
\OFkeyword{tolerance}および\OFkeyword{relTol}といった許容誤差は
...
以下で非圧縮定常状態層流の
\OFtool{simpleFoam}のチュートリアルの例で使われている緩和係数を参照できます．
\begin{OFverbatim}[file, linenum=17]
