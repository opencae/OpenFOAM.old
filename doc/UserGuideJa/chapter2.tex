...
以前のバージョンでは\OFtool{FoamX}というGUIケースエディタがありましたが，
利用者がエディタによって編集できるファイルを好んでおり，
メンテナンスをあまりしないためバージョン1.5ではなくなりました．
...
\end{verbatim}
\end{OFterminal}
\OFtool{blockMesh}の実行状況はターミナルウインドウに表示されます．
\index{blockMeshDict@\OFdictionary{blockMeshDict}!ディクショナリ}%
\index{ディクショナリ!blockMeshDict@\OFdictionary{blockMeshDict}}%
...
二つのパッチが使用されています．
(1) キャビティの固定された側面と底面用の\OFkeyword{fixedwall}と，
(2) キャビティの駆動天井面用の\OFkeyword{movingwall}です．
どちらも$p$が\OFkeyword{zeroGradient}ですが，
これは圧力の勾配が$0$であるということです．
\OFkeyword{frontAndBack}は2次元の問題の場合の表裏の平面を示していて，
...
速度の境界条件は\OFkeyword{frontAndBack}パッチと同じ条件です．
\OFkeyword{fixedWall}に関してはすべりなしのため
...
\autoref{fig:6.1}に示すように\OFtool{ParaView}のウインドウが開きます．
\index{Pipeline Browser@\OFemph{Pipeline Browser}!ウィンドウ}%
\index{ウィンドウ!Pipeline Browser@\OFemph{Pipeline Browser}}%
\OFemph{Pipeline Browser}を見ると，\OFtool{ParaView}が\OFkeyword{cavity.foam}，
つまりキャビティケースのモデュールを開いていることが確認できます．
\emph{\OFemph{Apply}ボタンをクリックする前に}%
\index{Region Status@\OFemph{Region Status}!ウィンドウパネル}%
\index{ウィンドウパネル!Region Status@\OFemph{Region Status}}%
\OFemph{Region Status}とパネルから
表示する要素を選択する必要があります．
解析ケースが単純なので\OFemph{Region Status}パネルのチェックボックスで
全てのデータを選択することが簡単です．
パネル内の全要素を自動的にチェックすることができます．

\OFtool{ParaView}でジオメトリを読込むためには\OFemph{Apply}ボタンをクリックします．
\index{Display@\OFemph{Display}!ウィンドウパネル}%
\index{ウィンドウパネル!Display@\OFemph{Display}}%
\OFemph{Display}タブを開き選択したモジュールの表示形式を調整します．
\autoref{fig:2.3}に示すように，
(1) \OFkeyword{Color by}を\OFkeyword{Solid Color}に設定し，
(2) \OFemph{Set Solid Color}をクリックし適当な色（背景が白の場合は黒など）を選択，
(3)\ 
\index{Style@\OFemph{Style}!ウィンドウパネル}%
\index{ウィンドウパネル!Style@\OFemph{Style}}%
\OFemph{Style}パネルでは\OFkeyword{Representation}メニューから
\OFkeyword{Wireframe}を選択します．
背景色はトップメニューパネルで\OFkeyword{Edit}から
\index{View Settings...@\OFkeyword{View Settings...}!メニューエントリ}%
\index{メニューエントリ!View Settings...@\OFkeyword{View Settings...}}%
\OFkeyword{View Settings...}を選択して設定します．
...
\autoref{ssec:6.1.5}で述べるように視点操作を試してみましょう．
特に本ケースは2次元なので\OFkeyword{Edit}メニューの
\index{View Settings@\OFkeyword{View Settings}!メニューエントリ}%
\index{メニューエントリ!View Settings@\OFkeyword{View Settings}}%
\OFkeyword{View Settings}のGeneralパネルで
...
\OFtool{paraFoam}ウインドウに戻って，
\texttt{cavity.foam}ケースモジュールのために
\index{Properties@\OFemph{Properties}!ウィンドウパネル}%
\index{ウィンドウパネル!Properties@\OFemph{Properties}}%
\OFemph{Properties}パネルを選んでください．
ケースモジュールのためのパネルが存在していないようならば，
\texttt{cavity.foam}が黄色でハイライトされているか，
それと並んだ目のボタンに表示が有効であることを示す
スイッチは入っているか，
フォームのViewメニュー中でSourceが選ばれているか，
を確認してください．

見たいデータを表示する\OFtool{paraFoam}を準備するには，
最初に必須の実行時間として$0.5\unit{s}$分のデータを読込まなければなりません．
ケースが実行中で一方\OFtool{ParaView}を開いている場合，
時間ディレクトリの出力データは\OFtool{ParaView}に自動的にロードはされません．
データをロードするためには，\OFemph{Properties}画面で
\index{Update GUI@\OFemph{Update GUI}!ボタン}%
\index{ボタン!Update GUI@\OFemph{Update GUI}}%
\OFemph{Update GUI}を選択し，
緑のApplyボタンをクリックします． そうすれば時間データはロードされます．

\subsubsection{コンタプロット}
\label{sssec:2.1.4.1}
圧力を見るには
\index{Display@\OFemph{Display}!ウィンドウパネル}%
\index{ウィンドウパネル!Display@\OFemph{Display}}%
\OFemph{Display}パネルを開き，
選択したモジュールの表示形式を調整します．
圧力分布を見るには\autoref{fig:2.4}に示すように
StylePanelのRepresentationメニューをsurfaceにして
ColorPanelのSet Color byを$\circ$p，
そして
\index{Rescale to Data Range@\OFemph{Rescale to Data Range}!ボタン}%
\index{ボタン!Rescale to Data Range@\OFemph{Rescale to Data Range}}%
\OFemph{Rescale to Data Range}ボタンをクリックし，
メニューバーの下のツールバーにある
\index{VCR Controls@\OFkeyword{VCR Controls}!メニュー}%
\index{メニュー!VCR Controls@\OFkeyword{VCR Controls}}%
\OFkeyword{VCR Controls}または
...
\OFkeyword{Current Time Controls}で現在時刻を0.5にして$t = 0.5\unit{s}$における解析結果を表示します．
それらのパネルは\autoref{fig:6.4}に示すように
\OFtool{ParaView}ウィンドウのトップメニューの下にあります．
...
もしColor byメニューからセルアイコン \includegraphics{icon-cell-p} を選択しなければ
...
\OFkeyword{View}メニューから
\index{Show Color Legend@\OFkeyword{Show Color Legend}!メニューエントリ}%
\index{メニューエントリ!Show Color Legend@\OFkeyword{Show Color Legend}}%
\OFkeyword{Show Color Legend}を選択することで，
カラーバーを表示させることができます．
...
通常は\OFemph{Display}パネルで\OFkeyword{Color by U}を選択して大きさに応じた色付けをします．
\OFkeyword{Edit Color Map}を\OFemph{Show Color Legend}に設定し，
速度の凡例を表示させましょう．出力結果は\autoref{fig:2.7}のようになります．
\index{Color Legend@\OFemph{Color Legend}!ウィンドウ}%
\index{ウィンドウ!Color Legend@\OFemph{Color Legend}}%
\OFemph{Color Legend}（凡例）にはTimes Romanフォントが使用され，
...
Pipeline Browserで\texttt{cavity.foam}モジュールをハイライトした状態で，
FilterメニューからStream Tracerを選択し，Applyをクリックします．
そうすると，\autoref{fig:2.8}に示すように
Propatiesウィンドウが現れます．
Seed点は，ジオメトリの中心を垂直に通って，
Line Sourseに沿うように (例えば$(0.05, 0, 0.005)$から$(0.05, 0.1, 0.005)$
まで) 指定しましょう．このガイドに掲載した図ではPoint Resolutionを21に，
Max.PropagationをLengthで0.5に，Init.Step LenをCell Lengthで0.01に，
Integration DirectionをBOTHという設定を行いました．
また，Runge-Kutta 2 Integrator Typeはデフォルトパラメータを使いました．

Applyをクリックすると，トレーサが生成されます．そ
こでFilterメニューからTubesを選択することで，
高品質の流線図を作ることができます．
このレポートでは，次の設定を使いました．
Num.sidesを20，Radiusを0.003，Radius factorを10にしました．
Acceptを押すことで，\autoref{fig:2.9}ができます．
...
このケースでは$0.1\unit{s}$ごとの出力を指定します．
...
他のモジュールであるので，
\OFtool{ParaView}で同時に複数のケースを開くことができます．
若干不便なことには，\OFtool{ParaView}で新しいケースを開けるときには，
選ばれたデータが拡張子を含むファイル名である必要があります．
しかし，OpenFOAMにおいて，
各々のケースは特定のディレクトリ構造の中に拡張子なしで
複数のファイルに保存されます．
解決方法として，\OFtool{paraFoam}スクリプトが自動的に拡張子
`.OpenFOAM' が付いたダミーファイルを作成することになっています．
それゆえに，cavityケースモジュールは
cavity.OpenFOAMと呼ばれています．

\OFtool{ParaView}内から他のケースディレクトリを開けたいならば，
...
こうしてFileメニューからOpen Dataを選んでディレクトリツリーをたどり，
\texttt{cavityFine.foam}を選ぶことで，
cavityFineケースをParaViewに読み込めるようになりました．
さて，\OFtool{ParaView}で精密なメッシュの結果の
ベクトルプロットを作ることができます．
...
\end{verbatim}
\end{OFterminal}
処理を規定する\texttt{<calcType>}には\texttt{div}，
\texttt{components}，\texttt{mag}，
\texttt{magGrad}，\texttt{magSqr}を指定することができます．
...
\OFkeyword{comonents}および\OFkeyword{mag}の\OFkeyword{calcType}はスカラ速度を計測するのに有用です．
...
例として次のように入力し\OFcase{cavity}に対して実行します．
\begin{OFterminal}
\begin{verbatim}
foamCalc components U -case $FOAM_RUN/tutorials1.5/icoFoam/cavity
\end{verbatim}%$
\end{OFterminal}
それぞれのコンポーネントが\OFtool{ParaView}内でグラフとして描画されます．
...
\OFtool{ParaView}に読み込ませる必要があります．
作業をしている基本のモジュール，
この場合\texttt{cavity.foam}のPropertiesパネルの上部にある
Update GUIボタンをチェックしします．
ApplyをクリックすることでVol Field Statusウインドウに
新しいデータが読み込まれます．
新しいデータの選択と変更の適用を確認し，
必要ならApplyをもう一度クリックします．
境界領域がRegion Statusパネルで選択されていると
境界部分のデータ補間が不適切に行われています．
したがって\OFkeyword{movingwall}や\OFkeyword{fixedwall}，\OFkeyword{frontAndBack}といった
Region Statusパネルのパッチの選択を解除して変更を適用します．

さて，\OFtool{ParaView}の図を描画しましょう．
まずは描画したいモジュールを選択し，
\index{Plot Over Line@\OFkeyword{Plot Over Line}!メニューエントリ}%
\index{メニューエントリ!Plot Over Line@\OFkeyword{Plot Over Line}}%
\OFkeyword{Plot Over Line}フィルタをFilter $\rightarrow$ Data Analisysから選択します．
...
ApplyをクリックするとXY Plotウインドウに図が描画されます．
Displayパネルの
\index{Plot Type@\OFkeyword{Plot Type}!メニュー}%
\index{メニュー!Plot Type@\OFkeyword{Plot Type}}%
\OFkeyword{Plot Type}をScatter Plotに設定し，
Attribute Mode Point Dataにします．
Use Data Arrayオプションの\verb|arc_length|でX軸データがキャビティの
底からの距離になります．

DisplaysウインドウのLine Seriesパネルから
表示するデータを選択することができます．
表示されているスカラ場のリストから，
...
各軸のタイトルや形式の設定をする
\index{Chart Options@\OFemph{Chart Options}!ウィンドウ}%
\index{ウィンドウ!Chart Options@\OFemph{Chart Options}}%
\OFemph{Chart Options}ウインドウが表示されます．
各軸のメニューはダブルクリックをしてLayout and Tytleにすることで
拡大することができます．
フォントや色，軸名の位置，軸の値の範囲や線形か対数かといった
設定が行うことができます．
\autoref{fig:2.11}は\OFtool{ParaView}によって作画された図です．
望みどおりのグラフが作成できます．
...
\OFpath{\$FOAM\_RUN/tutorials/icoFoam}ディレクトリに入れておきました．
...
乱流のための追加の方程式およびモデルは\OFtool{turbFoam}と呼ばれる
OpenFOAMソルバにおいて実行されます．

\subsubsection{前処理}
\label{sssec:2.1.8.1}
\OFpath{\$FOAM\_RUN/tutorials/turbFoam}ディレクトリの
...
$k$と$\varepsilon$のファイル (\OFpath{0/k}と\OFpath{0/epsilon}) を開き，
境界条件を確かめます．
壁タイプの境界条件の選択には，
$\varepsilon$については\OFkeyword{zeroGradient}境界条件を，
$k$については\OFkeyword{fixedValue 0}を指定します．
いかにして$k$と$\varepsilon$の初期値を決めるでしょうか．
しかし，$k$，$\varepsilon$については，
解法アルゴリズムにてゼロ除算を避けるために，非ゼロの値を与えます．

$k$，$\varepsilon$の適当な初期条件は，
速度変動$\bm{U}'$と
\index{らんりゅう@乱流!ながさスケール@長さスケール}%
乱流長さスケール$l$を用いて設定することができ，
次式に示すように表されます．
\begin{align}
 \label{eq:2.8}
...
\OFdictionary{RASProperties}ディクショナリを開き，RAS乱流モデルを選択します．
\index{らんりゅう@乱流!モデル}%
乱流モデルは\OFkeyword{RASModel}エントリで選択されます．
\autoref{tbl:3.9}に示すように
多くの使用可能なモデルが与えられています．
ユーザは標準$k$--$\varepsilon$モデルを表す\OFkeyword{kEpsilon}をここでは選択します．
そして，\OFkeyword{turbulence}のスイッチを\OFkeyword{on}にします．
乱流モデルに関する係数は標準ディクショナリの\OFkeyword{kEpsilonCoeffs}以下に，
また，同ディクショナリに\OFkeyword{wallFunctionCoeffs}の設定もあります．
...
\subsubsection{コードの実行}
\label{sssec:2.1.8.2}
ケースディレクトリに入り，\texttt{turbFoam}とタイプすることで
\OFtool{turbFoam}を実行します．
粘性が小さいこの計算ケースでは，
...
本操作は，\OFtool{turbFoam}が最新のディレクトリ，例えば\OFpath{10.0}，
...
物理量の適用の過程を明確にするために，
元となるケース\OFcase{cavity}で\OFkeyword{movingWall}であった
\OFkeyword{upper wall}は\OFkeyword{lid}という名前に変更されています．
...
\OFkeyword{patchMap}と\OFkeyword{cuttingPatches}という二つの入力項目があります．
\OFkeyword{patchMap}リストは元となる物理量のパッチとマッピング対象となる
物理量のパッチを含みます．対象物理量のパッチに
元となる物理量のパッチの値を引き継ぎたいときに利用します．
\OFcase{cavityClipped}において\OFkeyword{lid}の境界値を
\OFcase{cavity}の\OFkeyword{movingWall}から
...
本ケースでは，\OFkeyword{fixedWalls}を内挿プロセスの実例説明に用いることとします．
\begin{OFfile}
\begin{verbatim}
cuttingPatches
(
...
この実例において，\OFkeyword{fixedWalls}パッチの速度を$(0, 0, 0)$に
リセットしたい場合があります．このときは，\OFpath{U}をエディタで開き，
\OFkeyword{fixedWalls}を\OFkeyword{nonuniform}から
\OFkeyword{uniform (0,0,0)}に変更します．
そして，\OFtool{icoFoam}を実行すればよいです．
...
\OFkeyword{lid}と\OFkeyword{fixedWalls}を，ハイライトすることができます．
...
\OFpath{\$FOAM\_RUN/tutorials/solidDisplacementFoam}ディレクトリの
\OFpath{plateHole}ケースに移動し，\OFpath{plateHole}ケースの
\OFpath{constant/polyMesh/blockMeshDict}をエディタで開きます．
\OFdictionary{blockMeshDict}ディクショナリのエントリを以下に示します．
\begin{OFfile}
\begin{verbatim}
// * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * //
...
そのうち左の面 (\OFkeyword{left}) と下の面 (\OFkeyword{down}) は対称面です．
...
使って定義するとよいでしょう．
\OFkeyword{frontAndBack}パッチは2次元問題の場合は
無視される面を示しています．
...
\OFkeyword{left}と\OFkeyword{down}のパッチは
\OFkeyword{type}が\OFkeyword{symmetryPlane}である必要があります．
同様に\OFkeyword{frontAndBack}は\OFkeyword{empty}になります．
...
の線形結合で指定されます．
\OFkeyword{up}および\OFkeyword{hole}パッチは表面力ゼロであるため，
表面力ベクトルおよび圧力ともにゼロが設定されます．
\OFkeyword{right}パッチについては，\autoref{fig:2.24}に示すように，
...
まず\OFsubdictionary{solvers}サブディクショナリを見て，
\OFrevision{要再訳}%%
\OFkeyword{U}のソルバとして\OFkeyword{ICCG}を選択してください．
...
\subsection{格子の生成}
\label{ssec:2.3.1}
\OFpath{\$FOAM\_RUN/tutorials/interFoam}にある
\OFpath{damBreak}のケースディレクトリに移動しましょう．
前に述べた方法で\OFtool{blockMesh}を走らせて格子を生成してください．
この\OFtool{damBreak}の格子は五つのブロックで構成されます．
...
\OFkeyword{leftWall}，\OFkeyword{rightWall}，\OFkeyword{lowerWall}，
\OFkeyword{atmosphere}，\OFkeyword{defaultFaces}の
五つの境界パッチがあります．
パッチの種類について理解しておきましょう．
\OFkeyword{atmosphere}は何の属性もなく，
単に境界条件によって規定される標準の\OFboundary{patch}です．
\OFkeyword{defaultFaces}は，本ケースでは2次元であるため
パッチの法線方向を解析の対象としないため，\OFboundary{empty}とします．
\OFkeyword{leftWall}，\OFkeyword{rightWall}，
\OFkeyword{lowerWall}はそれぞれ
...
このモデルは\OFkeyword{gamma} ($\gamma$) 場の
\index{gammaContactAngle@\OFboundary{gammaContactAngle}!きょうかいじょうけん@境界条件}%
\index{きょうかいじょうけん@境界条件!gammaContactAngle@\OFboundary{gammaContactAngle}}%
\OFboundary{gammaContactAngle}の境界条件と関連付けられています．
その場合，静的な接触角度\OFkeyword{theta0} $\theta_{0}$や，
前縁や後縁における動的な接触角度である
\OFkeyword{thetaA} $\theta_{\mathrm{A}}$と\OFkeyword{thetaR} $\theta_{\mathrm{B}}$，
...
この場合，\OFkeyword{gamma}に対して\OFkeyword{gammaContact}の境界条件を設定する代わり，
\OFkeyword{zeroGradient}の境界条件を課すことになります．

\OFkeyword{top}の境界は大気に対して開放されていることから，
\OFkeyword{atmosphere}タイプの境界条件を与えます．
また，この2次元問題において，前後の面のような
\OFkeyword{defaultFaces}の境界条件は通常通り\OFkeyword{empty}タイプです．

\subsection{初期条件の設定}
\label{ssec:2.3.3}
これまでのケースと異なり，ここでは相比率$\gamma$に対して，
以下のような非一様な初期条件を与えます．
\begin{align}
 \label{eq:2.15}
 \gamma =
...
また，この領域における相比率$\gamma$を$1$と指定しています．

さて，
\index{setFields@\OFtool{setFields}!ユーティリティ}%
\index{ユーティリティ!setFields@\OFtool{setFields}}%
\OFtool{setFields}を他のプログラムと同様に起動してください．
そうしたら，\OFtool{paraFoam}を用いて，初期の\OFkeyword{gamma}場が
...
\begin{figure}[ht]
 ...
 \caption{相比率\OFkeyword{gamma}の初期条件}
 \label{fig:2.21}
\end{figure}
...
このファイルは，\OFkeyword{phase1}と\OFkeyword{phase2}に分かれて，
各流体の物性値を保持しています．
...
\index{environmentalProperties@\OFpath{environmentalProperties}!ファイル}%
\index{ファイル!environmentalProperties@\OFpath{environmentalProperties}}%
\OFpath{environmentalProperties}のディクショナリは，
重力加速度ベクトルを指定していますが，
このチュートリアルでは，
この値は$(0, 9.81, 0)\unit{m\,s^{-2}}$としてください．
...
流体の
\index{ちょくせつすうちシミュレーション@直接数値シミュレーション}%
...
風上の設定は，\OFdictionary{fvSchemes}ディクショナリの
\OFsubdictionary{divSchemes}サブディクショナリ内に作成されており，
運動量方程式における$\nabla \cdot (\rho\phi\bm{U})$項に対応する
\OFkeyword{div(rho*phi,U)}キーワードは，
\OFkeyword{Gauss upwind}を読みこみます．
相$\gamma$に対する二つの方程式における対流項にはいくつかの注意を必要とします．
適切な精度を維持しつつ有界性を保証しなくてはならないため，
対流項内での補完間のためには，
有界なスカラに対するリミッタ付きの線形なスキームである
\OFkeyword{limitedLinear01}を用います．

リミッタ付きの線形なスキームは
\autoref{ssec:4.4.1}に記述される係数を必要とします．
ここで安定性を最重視して，
キーワード\OFkeyword{div(phi,gamma)}に対応する$\nabla \cdot (\phi\gamma)$項，
キーワード\OFkeyword{div(phirb,gamma)}に対応する$\nabla \cdot (\phi_{\mathrm{rb}}\gamma)$項で
$\phi = 1.0$の状態を選びます．
...
\index{fvSchemes@\OFdictionary{fvSchemes}!ディクショナリ}%
\index{ディクショナリ!fvSchemes@\OFdictionary{fvSchemes}}%
\OFdictionary{fvSchemes}ディクショナリのエントリは以下のようになるべきです．
\begin{OFfile}
\begin{verbatim}
// * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * //
...
特に興味深いものは\OFkeyword{nGammaSubCycles}と\OFkeyword{cGamma}キーワードです．
\index{nGammaSubCycles@\OFkeyword{nGammaSubCycles}!キーワード}%
\index{キーワード!nGammaSubCycles@\OFkeyword{nGammaSubCycles}}%
\OFkeyword{nGammaSubCycles}は$\gamma$方程式内の内側反復の数を表してあり，
ここで，内側反復は与えられた時間ステップ内での方程式に対する
付加的な解の点数です．
それは，時間ステップや計算時間の莫大な増加なしで
解を安定させることができるようにするものです．
ここでは，四つのsub-cycleを指定しており，
$\gamma$方程式は実際の各時間ステップ内で4分の1の幅の
時間スッテプで4回解かれていることを意味します．

\index{cGamma@\OFkeyword{cGamma}!キーワード}%
\index{キーワード!cGamma@\OFkeyword{cGamma}}%
\OFkeyword{cGamma}キーワードは界面の圧縮を制御する要素です．
...
\end{verbatim}%$
\end{OFterminal}
コードは，出力のコピーを\OFpath{log}ファイル内に記録しつつ，
対話形式で実行されます．
...
参照時間の経過とともに相比率\OFkeyword{gamma}の発達を見ることができます．
...
時刻\OFpath{0}のディレクトリ内の\OFpath{gamma}という相の場を
再初期化しなければなりません．
というのも\OFkeyword{gamma}は新しい格子とは合致しない
いくつかの要素を含んでいるからです．
...
つまり，その要素が$\gamma = 1$か$\gamma = 0$をもつようにです．
\OFtool{mapFields}でのフィールドの更新は界面に補間された値$0 < \gamma < 1$が
生成されるかもしれないので，
\OFtool{setFields}ユーティリティを以下のように再実行したほうがよいでしょう．
その前に初期条件の一様な$\gamma$のバックアップファイル
\OFpath{0/gamma.org}を\OFpath{0/gamma}にコピーします．
\begin{OFterminal}
\begin{verbatim}
...
\begin{figure}[ht]
 ...
 (b) $t = 0.50\unit{s}$のとき
 \caption{正確なメッシュの$\gamma$相のスナップショット}
 \label{fig:2.24}
\end{figure}
...
\end{verbatim}
\end{OFterminal}
すると\OFkeyword{processor1}はParaViewのケースモジュールとして表れます．
\OFkeyword{simple}方式を使った領域分割を行った
\OFkeyword{processor1}から\autoref{fig:2.23}のような格子が見えます．
