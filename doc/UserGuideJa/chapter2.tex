...
\autoref{fig:6.1}に示すように\OFtool{ParaView}のウィンドウが開きます．
\index{Pipeline Browser@\OFemph{Pipeline Browser}!ウィンドウ}%
\index{ウィンドウ!Pipeline Browser@\OFemph{Pipeline Browser}}%
\OFemph{Pipeline Browser}を見ると，\OFtool{ParaView}が\OFkeyword{cavity.OpenFOAM}，
つまりキャビティケースのモデュールを開いていることが確認できます．
\emph{\OFemph{Apply}ボタンをクリックする前に}%
\index{Mesh Parts@\OFemph{Mesh Parts}!ウィンドウパネル}%
\index{ウィンドウパネル!Mesh Parts@\OFemph{Mesh Parts}}%
\OFemph{Mesh Parts}パネルから
表示する要素を選択する必要があります．
解析ケースが単純なので\OFemph{Mesh Parts}パネルのチェックボックスで
全てのデータを選択することが簡単です．
...
データをロードするためには，\OFemph{Properties}ウィンドウで
\index{Refresh Times@\OFemph{Refresh Times}!ボタン}%
\index{ボタン!Refresh Times@\OFemph{Refresh Times}}%
\OFemph{Refresh Times}をクリックします．
これで各時刻のデータが\OFtool{ParaView}にロードされます．
...
\OFemph{Style}パネルのRepresentationメニューをsurfaceにして
ColorPanelのSet Color byを \includegraphics{icon-point-p}，
そして
...
Colorパネルで \includegraphics{icon-point-p}，
...
これは，\OFemph{Color Scale Editor}で\OFemph{Choose Preset}を選び，
\OFemph{Blue to Red Rainbow}を選択することで変更できます．
\OFemph{OK}ボタンで確定したあとに，\OFemph{Make Default}を押せば
...
基準点は$(0, 0, 1)$とします (\OFemph{Z Normal}ボタンをクリックします)．
...
\OFemph{Pipeline Browser}でそれらのモジュールを選択し，
Properties PanelのDeleteをクリックして削除するか，
\OFemph{Pipeline Browser}で目の形のボタンをクリックして
それらのモジュールを非表示にします．

各格子の中心におけるベクトルグラフを作成することにしましょう．
まず，\autoref{sssec:6.1.7.1}に述べるように
格子の中心のデータのみに絞り込みます．
\OFemph{Pipeline Browser}上で強調表示されている
cavity.OpenFOAMのモジュールを選択し，
Filter $\rightarrow$ AlphabeticalメニューからCell Centersを選択してApplyをクリックします．

\OFemph{Pipeline Browser}でCentersが強調表示された状態で，
Filter $\rightarrow$ AlphabeticalメニューからGlyphを選択します．
\autoref{fig:2.6}のような\OFemph{Properties}ウィンドウが表示されます．
この\OFemph{Properties}パネルの\OFkeyword{vectors}メニューでは，
ベクトル場は速度のみなので，速度場\OFkeyword{U}が自動的に選択されています．
\OFkeyword{Scale Mode}は速度の\OFkeyword{Vector Magnitude}が初期値として選択されていますが，
領域全体を通る速度の様子を見るために，\OFkeyword{off}を選択し，
\OFemph{Set Scale Factor}に0.005をにします．
\OFemph{Apply}をクリックするとベクトルが表示されますが，単色，例えば白になっているでしょう．
通常は\OFemph{Display}パネルで\OFkeyword{Color by U}を選択して速度に応じた色付けをします．
\OFkeyword{Edit Color Map}の中で\OFemph{Show Color Legend}を選択し，
...
\OFemph{Automatic Label Format}を解除して\OFemph{Label Format}テキストボックスに
\verb|%-#6.2f|を入力することで二つの有効数字でラベルを固定しています．
背景色は，\autoref{sssec:6.1.5.1}で述べるように，
\OFkeyword{View Settings}の\OFemph{General}パネルで白に設定されています．

左右の壁において，ベトルが壁面を通り抜けるように見えていることに注意してください．
しかし，さらによく調べると，この壁に垂直な方向を向いている速度は$0$であることがわかります．
この少し混乱する状態は，スケーリングが\OFkeyword{off}で速度が$0$のとき，
\OFtool{ParaView}は$x$方向のベクトルで表示するということに起因します．
...
\OFemph{Pipeline Browser}で\texttt{cavity.OpenFOAM}モジュールをハイライトした状態で，
\OFemph{Filter}メニューから\OFemph{Stream Tracer}を選択し，\OFemph{Apply}をクリックします．
そうすると，\autoref{fig:2.8}に示すように
\OFemph{Propaties}ウィンドウが現れます．
\OFemph{Seed}の点は，ジオメトリの中心を垂直に通って，
\OFkeyword{Line Sourse}に沿うように (例えば$(0.05, 0, 0.005)$から
$(0.05, 0.1, 0.005)$まで) 指定しましょう．
このガイドに掲載した図では\OFemph{Point Resolution}を21に，
\OFemph{Max Propagation}を\OFkeyword{Length}で0.5に，
\OFemph{Initial Step Length}を\OFkeyword{Cell Length}で0.01に，
\OFemph{Integration Direction}を\OFkeyword{BOTH}という設定を行いました．
また，\OFkeyword{Runge-Kutta 2} \OFemph{Integrator Type}は
デフォルトパラメータで使いました．

\OFemph{Apply}をクリックすると，トレーサが生成されます．
そこで\OFemph{Filter}メニューから\OFkeyword{Tubes}を選択することで，
高品質の流線図を作ることができます．
このレポートでは，次の設定を使いました．
\OFemph{Num.\ sides}を20，\OFemph{Radius}を0.003，\OFemph{Radius factor}を10にしました．
\OFemph{Accept}を押すことで，\autoref{fig:2.9}ができます．
...
\end{OFverbatim}%$
それぞれの成分が\OFtool{ParaView}内でグラフとして描画されます．
...
これにより，\OFtool{ParaView}に新しいフィールドが読み込まれ，
...
\OFkeyword{movingwall}や\OFkeyword{fixedwall}，\OFkeyword{frontAndBack}といった
パッチの選択を解除して，変更を適用します．

さて，\OFtool{ParaView}でグラフを表示してみましょう．
まずは描画したいモジュールを選択し，
...
3D Viewウィンドウの下または横に新しいXY Plotウィンドウが開きます．
Propertiesウィンドウで線の終点を指定すると
Plobelineモジュールが作成されます．
この例ではPoint1を$(0.05, 0, 0.005)$，
Point2を$(0.05, 0.1, 0.005)$と指定して線を領域の中心の真上におきます．
Resolutionは100まで設定できます．

ApplyをクリックするとXY Plotウィンドウにグラフが描画されます．
Displayパネルで，
\index{Attribute Mode@\OFkeyword{Attribute Mode}!メニュー}%
\index{メニュー!Attribute Mode@\OFkeyword{Attribute Mode}}%
\texttt{Attribute Mode}を\texttt{Point Data}に設定します．
\texttt{Use Data Array}オプションを\texttt{X Axis Data}にし，
\verb|arc_length|オプションを加えて，
グラフの$x$軸データがキャビティの底からの距離になるようにできます．

DisplaysウィンドウのLine Seriesパネルから
表示するデータを選択することができます．
表示されているスカラ場のリストから，
...
グラフの体裁を整えるには，\OFemph{Line Series}パネルの下にある設定，
\texttt{Line Color}，\texttt{Line Thickness}，\texttt{Line Style}，
\texttt{Marker Style}，そして\texttt{Chart Axes}を変更します．

また，\OFemph{XY Plot}の左上にあるボタンをクリックすることもできます．
例えば，3番目のボタンでは，それぞれの軸のタイトルや凡例などを設定する
\OFemph{View Settings}を制御することができます．
また，軸のタイトルのフォント，色，配置，
...
このグラフは点で表示していますが，
\OFemph{Display}ウィンドウで
\index{Enable Line Series@\OFemph{Enable Line Series}!ボタン}%
\index{ボタン!Enable Line Series@\OFemph{Enable Line Series}}%
\OFemph{Enable Line Series}ボタンを
有効にすれば線で表示できます．
注：もしこのボタンが，グレー表示で無効の状態になっていたら，
\OFemph{Line Series}パネルでどれか変数を選択すれば有効になります．
\OFemph{Enable Line Series}ボタンを選択しておけば，
\index{Line Style@\OFkeyword{Line Style}!メニュー}%
\index{メニュー!Line Style@\OFkeyword{Line Style}}%
\OFemph{Line Style}や
\index{Marker Style@\OFkeyword{Marker Style}!メニュー}%
\index{メニュー!Marker Style@\OFkeyword{Marker Style}}%
\OFemph{Marker Style}もユーザの好みで調整できます．
...
// ************************************************************************* //
\end{OFverbatim}
このケースでは，\OFkeyword{movingWall}と\OFkeyword{fixedWalls}のパッチに対して
...
2次元問題における前後の面を表している\OFkeyword{defaultFaces}パッチは，
...
ここで安定性を最重視して，
キーワード\OFkeyword{div(phi,alpha)}に対応する$\nabla \cdot (\bm{U}\alpha_{1})$項，
キーワード\OFkeyword{div(phirb,alpha)}に対応する$\nabla \cdot (\bm{U}_{\mathrm{rb}}\alpha_{1})$項で
$\phi = 1.0$の状態を選びます．
\OFrevision{要再訳}
...
特に興味深いものは\OFkeyword{nAlphaSubCycles}と\OFkeyword{cAlpha}キーワードです．
...
以下を試してください．
\texttt{tee}によって計算内容をターミナルウィンドウに表示しつつ，
ログファイルに記録します．
\begin{OFverbatim}[terminal]
