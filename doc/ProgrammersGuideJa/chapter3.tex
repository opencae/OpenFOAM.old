...
\index{tutorials@\OFpath{tutorials}}%  
\index{tutorials directory@\OFpath{tutorials} directory}%  
...
\index{れいだい@例題!まわりのながれ@まわりの流れ}%  
円柱まわりのポテンシャル流れを調べます．
この例題にはOpenFOAMの以下のような特徴を紹介します．
\begin{itemize}
\index{めっしゅ@メッシュ!ひちょっこうこうし@非直交格子}%  
 \item 非直交格子
...
\index{potentialFoam そるば@\OFclass{potentialFoam} ソルバ}% 
\index{potentialFoam@\OFclass{potentialFoam}}% 
...
\subsection{メッシュ生成}
\label{ssec:3.1.3}
\index{blockMesh@\OFtool{blockMesh}!キーワード}%
\index{キーワード!blockMesh@\OFtool{blockMesh}}%
\OFtool{blockMesh}を使ったメッシュ生成はユーザ・ガイドで解説されています．
...
\index{れいだい@例題!ばっくすてっぷじょうのていじょうらんりゅう@バック・ステップ上の定常乱流}%
\index{ばっくすてっぷじょうのていじょうらんりゅう@バック・ステップ上の定常乱流}%
\label{sec:3.2}
...
\index{incompressibleTurbulenceModels!ライブラリ}%
\index{ライブラリ!incompressibleTurbulenceModels}%
ライブラリの中の乱流モデル，
および\OFclass{incompressibleTransportModels}
\index{incompressibleTransportModels!ライブラリ}%
\index{ライブラリ!incompressibleTransportModels}%
ライブラリの中の非ニュートン流体モデル
の全てを適用することができます．
...
\index{かあつされたみずたんくのげんあつ@加圧された水タンクの減圧}%
\index{れいだい@例題!かあつされたみずたんくのげんあつ@加圧された水タンクの減圧}%
\label{sec:3.4}
 ...
 \item[支配方程式] この問題では，有限の速度で伝播する波を解くために，
            ...
            \begin{itemize}
             ...
             \item ニュートン流体の運動量方程式
                   ...
                   \end{align}
            \end{itemize}
 \index{empty!きょうかいじょうけん@境界条件}%
 \index{pressureOutlet!きょうかいじょうけん@境界条件}%
 \index{wall!きょうかいじょうけん@境界条件}%
...
\end{figure}
 ...
 \item[支配方程式] \mbox{}
            ...
            \item[境界条件] \mbox{}
            \index{empty@きょうかいじょうけん@境界条件}%
            \index{inlet@きょうかいじょうけん@境界条件}%
            \index{outlet@きょうかいじょうけん@境界条件}%
            \begin{itemize}
             \item \OFkeyword{inlet}
             \index{きょうかいじょうけん@境界条件!inlet@\OFkeyword{inlet}}%
					は速度固定$\bm{U} = (0,\ 0,\ 0) \unit{m/s}$の
                   \OFboundary{inlet}条件として指定%
\footnotemark
             \item \OFkeyword{outlet}
...
他方を\OFboundary{zeroGradient}とする．}%
             ...
             \item \OFkeyword{lowerWall}は$\bm{B} = (0,\ 20,\ 0) \unit{T}$の
                   \OFboundary{wall}として指定
             \item \OFkeyword{front}および\OFkeyword{back}境界は
                   \index{きょうかいじょうけん@境界条件!\OFboundary{empty}}%
                   \OFboundary{empty}として指定
            \end{itemize}
