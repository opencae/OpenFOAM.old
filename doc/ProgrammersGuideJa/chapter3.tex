...
例題は，インストールしたOpenFOAMの\OFpath{tutorials}サブディレクトリ
...
\section{円柱まわりの流れ}
\label{sec:3.1}
この例題では，\OFtool{potentialFoam}を使って円柱まわりのポテンシャル流れを調べます．
...
\begin{itemize}
 \item 完全なメッシュの勾配付け機能を利用した，\OFtool{blockMesh}によるメッシュ生成
 \item 定常乱流
\end{itemize}
...
\end{figure}
 ...
 \item[乱流モデル] \mbox{}
            ...
            \end{itemize}
 \item[ソルバ名] \OFtool{simpleFoam}: 定常非圧縮性流れ用．
...
\OFclass{incompressibleTurbulenceModels}ライブラリの中の乱流モデル，
および\OFclass{incompressibleTransportModels}ライブラリの中の非ニュートン流体モデル
の全てを適用することができます．
...
現行バージョンでは\OFpath{\$FOAM\_TUTORIALS/compressible/sonicFoam}にある．}%
 ...
 \item[ソルバ名] \OFtool{sonicFoam}: 圧縮性の遷音速・超音速層流%
...
\end{figure}
 ...
 \item[境界条件] \OFtool{FoamX}を使って，以下のような物理境界条件を設定できます．
            \begin{itemize}
             \item \OFkeyword{outerWall}は\OFboundary{wall}条件として指定
             \item \OFkeyword{axis}は\OFboundary{symmetryPlane}として指定
...
数値・単位からして動粘性係数ではない．}%
 ...
 \item[熱力学特性] \mbox{}
            ...
            \end{itemize}
 \item[ソルバ名] \OFtool{sonicLiquidFoam}: 遷音速・超音速の圧縮性液体の層流コード．
...
$0.1 \unit{ms}$になっている．}%
参考に，ファイル\OFdictionary{controlDict}を以下に引用します．
\begin{OFverbatim}[file, linenum]
/*--------------------------------*- C++ -*----------------------------------*\
| =========                 |                                                 |
| \\      /  F ield         | OpenFOAM: The Open Source CFD Toolbox           |
...
この問題は，電磁流体力学 (MHD) として知られる，流体力学の一つの分野であり，\OFtool{mhdFoam}を使います．
...
他方を\OFboundary{zeroGradient}とする．}%
 ...
 \item[輸送特性] \mbox{}
            ...
            \end{itemize}
 \item[ソルバ名] \OFtool{mhdFoam}: 非圧縮性で層流の電磁流体コード
