...
\label{sec:2.1}
空間微分を定義する前に，
\index{えんざんし@演算子!ベクトル}%
\index{ベクトル!えんざんし@演算子}%
\OFemph{ベクトル演算子}ナブラ$\nabla$を紹介します．
...
\index{テンプレートクラス!List<Type>@\OFclass{List<Type>}}%
\index{List<Type>@\OFclass{List<Type>}!テンプレートクラス}%
OpenFOAMでは，テンソルクラスのリストは
\index{テンプレートクラス!Field<Type>@\OFclass{Field<Type>}}%
\index{Field<Type>@\OFclass{Field<Type>}!テンプレートクラス}%
テンプレートクラス\OFclass{Field<Type>}によって
標準で定義されています．
コードの視認性をより良くするために，
例えば\OFclass{Field<vector>}のような\OFclass{Field<Type>}のインスタンスは，
\texttt{typedef}定義により
\index{クラス!scalarField@\OFclass{scalarField}}%
\index{scalarFieldクラス@\OFclass{scalarField}クラス}%
\OFclass{scalarField}，
\index{クラス!vectorField@\OFclass{vectorField}}%
\index{vectorFieldクラス@\OFclass{vectorField}クラス}%
\OFclass{vectorField}，
\index{クラス!tensorField@\OFclass{tensorField}}%
\index{tensorFieldクラス@\OFclass{tensorField}クラス}%
\OFclass{tensorField}，
\index{クラス!symmTensorField@\OFclass{symmTensorField}}%
\index{symmTensorFieldクラス@\OFclass{symmTensorField}クラス}%
\OFclass{symmTensorField}，
\index{クラス!tensorThirdField@\OFclass{tensorThirdField}}%
\index{tensorThirdFieldクラス@\OFclass{tensorThirdField}クラス}%
\OFclass{tensorThirdField}，
\index{クラス!symmTensorThirdField@\OFclass{symmTensorThirdField}}%
\index{symmTensorThirdFieldクラス@\OFclass{symmTensorThirdField}クラス}%
...
まずはもっとも
\index{メッシュ!きほんてきな@基本的な}%
基本的なメッシュクラスである
\index{クラス!polyMesh@\OFclass{polyMesh}}%
\index{polyMeshクラス@\OFclass{polyMesh}クラス}%
...
\begin{description}
 ...
 \item[面] セルの面のリスト\OFclass{List<face>}，あるいは\OFclass{faceList}です．
            ここで，
\index{クラス!face@\OFclass{face}}%
\index{faceクラス@\OFclass{face}クラス}%
            \OFclass{face}クラスは
\index{クラス!pointField@\OFclass{pointField}}%
\index{pointFieldクラス@\OFclass{pointField}クラス}%
            \OFclass{pointField}に対応する頂点番号のリストで定義されます．
 \item[セル] セルのリスト\OFclass{List<cell>}あるいは\OFclass{cellList}です．
            ここで，
\index{クラス!cell@\OFclass{cell}}%
\index{cellクラス@\OFclass{cell}クラス}%
            \OFclass{cell}クラスは
            上記の\OFclass{faceList}に対応する面番号のリストで定義されます．
 \item[境界]
\index{クラス!polyBoundaryMesh@\OFclass{polyBoundaryMesh}}%
\index{polyBoundaryMeshクラス@\OFclass{polyBoundaryMesh}クラス}%
            \OFclass{polyBoundaryMesh}は，境界の異なる領域を表すパッチのリスト
\index{クラス!polyPatchlist@\OFclass{polyPatchList}}%
\index{polyPatchlistクラス@\OFclass{polyPatchList}クラス}%
            \OFclass{polyPatchList}から成り立っています．
            このような方法で，解析の際に各々のパッチに
            異なる境界条件を適用できるように境界が細分化されます．
            あらゆる
\index{クラス!polyPatch@\OFclass{polyPatch}}%
\index{polyPatchクラス@\OFclass{polyPatch}クラス}%
            \OFclass{polyPatch}の全ての面は
            \OFclass{faceList}の一つのブロックに保存されており，
            ブロックの最初と最後の面への参照が保存された
\index{クラス!slice@\OFclass{slice}}%
\index{sliceクラス@\OFclass{slice}クラス}%
...
\index{wordクラス@\OFclass{word}クラス}%
...
\index{ゆうげんたいせきほう@有限体積法!メッシュ}%
\index{メッシュ!ゆうげんたいせきほう@有限体積法}%
メッシュ形状に由来する固有のデータが使われます．
そのためOpenFOAMでは，\OFclass{polyMesh}クラスを拡張した
\index{クラス!fvMesh@\OFclass{fvMesh}}%
\index{fvMeshクラス@\OFclass{fvMesh}クラス}%
\OFclass{fvMesh}に，
...
これはOpenFOAMにおいては，
\index{FieldField<Type>@\OFclass{FieldField<Type>}!テンプレートクラス}%
\index{テンプレートクラス!FieldField<Type>@\OFclass{FieldField<Type>}}%
テンプレートクラス
...
\begin{description}
 ...
 \item[境界場] これは
\index{GeometricBoundaryField@\OFclass{GeometricBoundaryField}!テンプレートクラス}%
\index{テンプレートクラス!GeometricBoundaryField@\OFclass{GeometricBoundaryField}}%
...
\index{dimensionSetクラス@\OFclass{dimensionSet}クラス}%
            \OFclass{dimensionSet}です．
 \item[古い値] 時間微分の離散化には，前の時間ステップにおける場のデータが必要になります．
\index{geometricField<Type>@\OFclass{geometricField<Type>}!テンプレートクラス}%
\index{テンプレートクラス!geometricField<Type>@\OFclass{geometricField<Type>}}%
...
\index{pointField<Type>@\OFclass{pointField<Type>}!テンプレートクラス}%
\index{テンプレートクラス!pointField<Type>@\OFclass{pointField<Type>}}%
...
\begin{figure}[ht]
 ...
 \medskip
 (a)
\index{テンプレートクラス!volField<Type>@\OFclass{volField<Type>}}%
\index{volField<Type>@\OFclass{volField<Type>}!テンプレートクラス}%
 \OFclass{volField<Type>}\par
 \bigskip
 \includegraphics{fig-2-4-b}\par
 \medskip
 (b)
\index{テンプレートクラス!surfaceField<Type>@\OFclass{surfaceField<Type>}}%
\index{surfaceField<Type>@\OFclass{surfaceField<Type>}!テンプレートクラス}%
 \OFclass{surfaceField<Type>}\par
 \bigskip
...
したがって，独自のクラス
\index{fvMatrix@\OFclass{fvMatrix}!テンプレートクラス}%
\index{テンプレートクラス!fvMatrix@\OFclass{fvMatrix}}%
...
それぞれ静的な関数のクラス
\index{クラス!finiteVolumeMethod@\OFclass{finiteVolumeMethod}}%
\index{finiteVolumeMethodクラス@\OFclass{finiteVolumeMethod}クラス}%
\OFclass{finiteVolumeMethod}や
\index{クラス!finiteVolumeCalculus@\OFclass{finiteVolumeCalculus}}%
\index{finiteVolumeCalculusクラス@\OFclass{finiteVolumeCalculus}クラス}%
...
\index{クラス!fvm@\OFclass{fvm}}%
\index{fvmクラス@\OFclass{fvm}クラス}%
\OFclass{fvm}および
\index{クラス!fvc@\OFclass{fvc}}%
\index{fvcクラス@\OFclass{fvc}クラス}%
\OFclass{fvc}で使える，偏微分方程式の項を離散化する主な関数を
\autoref{tbl:2.2}にリストアップしています．
各項の有限体積法による離散化は，
\index{ガウスのていり@ガウスの定理}%
ガウスの定理により，
...
\index{クラス!fvSchemes@\OFclass{fvSchemes}}%
\index{fvSchemesクラス@\OFclass{fvSchemes}クラス}%
...
\index{Gammaさぶんすきーむ@Gamma差分スキーム}%
            ...
            それはほかのよく知られたスキーム，
\index{さぶん@差分!van Leer}%
\index{van Leerさぶん@van Leer差分}%
            van Leer，
\index{さぶん@差分!SUPERBEE}%
\index{SUPERBEEさぶん@SUPERBEE差分}%
            SUPERBEE，
\index{さぶん@差分!MINMOD}%
\index{MINMODさぶん@MINMOD差分}%
            MINMODなどを表しています．
\end{description}
...
\begin{description}
\index{さぶん@差分!オイラーのいんかいほう@オイラーの陰解法}%
\index{オイラーのいんかいほう@オイラーの陰解法!さぶん@差分}%
...
または入力ファイルの中の適切な\OFkeyword{gradScheme}キーワードに
連動した\OFkeyword{fvc::grad}関数を使うか，
いずれの方法でも評価できます．
\begin{description}
\index{こうばい@勾配!ガウススキーム}%
 \item[ガウス積分] は，\OFkeyword{fvc::grad}関数を
            \OFkeyword{gradScheme Gauss}と合わせて使うことで動作します．
...
時間積分は三つの方法で離散化できます．
\begin{description}
\index{オイラーのいんかいほう@オイラーの陰解法!じかんのりさんか@時間の離散化}%
\index{じかんのりさんか@時間の離散化!オイラーのいんかいほう@オイラーの陰解法}%
...
\index{ようかいほう@陽解法!じかんのりさんか@時間の離散化}%
\index{じかんのりさんか@時間の離散化!ようかいほう@陽解法}%
...
\index{クランク・ニコルソンほう@クランク・ニコルソン法}%
\index{じかんのりさんか@時間の離散化!クランク・ニコルソンほう@クランク・ニコルソン法}%
 \item[クランク・ニコルソン法]
            空間の項の離散化に台形公式を使い，
...
\label{ssec:2.5.1}
\index{じかんのりさんか@時間の離散化!OpenFOAMにおける}%
現在のところ，時間の離散化の取り扱いは，
...
\begin{description}
\index{きょうかいじょうけん@境界条件!いりぐち@入口}%
 \item[入口] 入口における速度場が与えられ，それと整合させるために，
            圧力の境界条件は勾配ゼロになります．
\index{きょうかいじょうけん@境界条件!でぐち@出口}%
 \item[出口] 出口における圧力場が与えられ，
            速度には勾配ゼロ境界条件が指定されます．
\index{きょうかいじょうけん@境界条件!すべりなしふしんとうせいへきめん@滑りなし不浸透性壁面}%
...
\begin{description}
\index{きょうかいじょうけん@境界条件!たいしょうめん@対称面}%
